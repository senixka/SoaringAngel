...
import com.mygdx.game.Bullets.Ice;
import com.mygdx.game.MyGame;
import com.mygdx.game.Weapon;

public class Icethrower extends Weapon {
    public static final Texture texture = new Texture(Gdx.files.internal("IceThrower.psd"));
    ...
    public void update(float delta) {
        ...
        if (time > 9 && time < 10) {
            new Ice(getVector(), getPers().x, getPers().y);
            new Ice(getVector(), getPers().x, getPers().y);
            new Ice(getVector(), getPers().x, getPers().y);
